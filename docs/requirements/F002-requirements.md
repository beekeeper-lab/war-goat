# F002: Brave Search Integration - Requirements
...
This feature adds intelligent content discovery capabilities to War Goat using the Brave Search MCP server. Users can discover related content based on their existing interests, find new learning resources through smart search suggestions, verify and enrich URLs with additional context, and perform deep dives into specific topics.

## Work Item

- **ID**: F002
- **Type**: Feature
- **Title**: Brave Search Integration - Content Discovery and URL Enrichment

## User Stories

### US-1: Find Related Content
As a War Goat user, I want to find content related to my existing interests so that I can expand my knowledge in areas I'm already learning about.

### US-2: Smart Content Discovery
As a user, I want War Goat to suggest new content based on my interests and learning patterns so that I can discover valuable resources I might have missed.

### US-3: Verify and Enrich URLs
As a user, I want to verify URLs I add and get additional context about them (related articles, news, videos) so that I have a richer understanding of the content.

### US-4: Topic Deep Dive
As a user, I want to search for content on specific topics from within War Goat so that I can quickly find and add new learning resources without leaving the app.
...
- **FR-3**: URL Enrichment - When adding a non-YouTube URL (article, webpage), the system queries Brave Search to provide:
  - Related articles and resources
  - Recent news about the topic
  - Alternative/similar content sources
  - Summary of the content (via Brave Summarizer)
...
- **NFR-1**: Performance - Search queries should return results within 3 seconds under normal network conditions.

- **NFR-2**: Reliability - Search failures (API errors, network issues) degrade gracefully with clear error messages; the app remains functional.

- **NFR-3**: Usability - Search interface follows existing War Goat UI patterns; results are scannable and actionable.

- **NFR-4**: Cost Efficiency - Implement result caching (15-minute TTL) to reduce API calls for repeated queries. Free tier supports 2,000 queries/month.

### Constraints

- **CON-1**: Requires Brave Search API key configured in environment (`BRAVE_API_KEY`).

- **CON-2**: Free tier API limits: 2,000 queries/month, rate limits apply.

- **CON-3**: Local search (businesses, POIs) requires Brave Pro plan; exclude from MVP.
...
- **External**:
  - Brave Search MCP server (`@modelcontextprotocol/server-brave-search` or `@brave/brave-search-mcp-server`)
...
| MCP server unavailability | Low | Medium | Graceful degradation; disable search features when unavailable |
...
- Q1: Which Brave Search tools to integrate? **Resolved**: Web, News, Video search + Summarizer for MVP. [Blocker: no]
...
2. **Search UI Pattern**: Modal vs dedicated page vs inline expansion?
   - Recommendation: Modal for quick access, consistent with AddInterestModal pattern
...
2. Create `server/services/brave-search.js` service layer wrapping MCP calls
